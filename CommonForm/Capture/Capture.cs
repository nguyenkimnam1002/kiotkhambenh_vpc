...
//
// Future improvements
//  - SampleGrabber functionality
//  - VMR9 option
//	- use profile for Windows Media audio/video formats
//
// Copyright (C) 2006, 2007 Hans Vosman
// ------------------------------------------------------------------
    ...
    public class Capture : ISampleGrabberCB
    {
        ...
        public Filter VideoCompressor
        {
            ...
            set
            {
                ...
                // In case of Audio via PCI or audio via video device,
                // the audio source seems to get lost, try to restore
                if (this.AudioSource == null)
                {
                    // No AudioSource but there are audio sources
                    if (this.AudioSources != null)
                    {
                        ...
                        {
#if DEBUG
                            Debug.WriteLine("AudioSource restored in VideoCompressor ...");
#endif
                        }
                    }
                }
        ...
        /// </summary>
        public TVFineTune Tuner { get { return (tuner); } }
        ...
        // Events
        public event HeFrame FrameEvent;
        ...
        protected TVFineTune tuner = null;						// Property Backer: TV Tuner
        ...
        public bool StartRC(Control pnlVideo)
        {
            //assertStopped();
            ...
            previewWindow = pnlVideo;
            // Re-render the graph (if necessary)
            renderGraph();

            // Start the filter graph: begin capturing
            Thread thread = new Thread(new ParameterizedThreadStart(this.RunMedia), 0);
            thread.Start();
            if (!thread.Join(TimeSpan.FromSeconds(1.0)))
            {
                thread.Abort();
                return false;
            }
        ...
        public void Stop()
        {
            ...
            {
                Thread thread = new Thread(new ParameterizedThreadStart(this.StopMedia), 0);
                thread.Start();
                if (!thread.Join(TimeSpan.FromSeconds(1.0)))
                {
                    thread.Abort();
        ...
        protected void createGraph()
        {
            ...
            if ((int)graphState < (int)GraphState.Created)
            {
                ...
                // Make a new filter graph
                Type typeFromCLSID = Type.GetTypeFromCLSID(Clsid.FilterGraph, true);
                graphBuilder = (IGraphBuilder)Activator.CreateInstance(typeFromCLSID);//Type.GetTypeFromCLSID(Clsid.FilterGraph, true));
                ...
                Type type = Type.GetTypeFromCLSID(Clsid.SampleGrabber);
                if (type == null)
                {
                ...
                cat = PinCategory.Capture;
                med = MediaType.Interleaved;
                iid = typeof(TVFineTune.IAMTVAudio).GUID;
                hr = captureGraphBuilder.FindInterface(
                    ref cat, ref med, videoDeviceFilter, ref iid, out o);
                if (hr != 0)
                {
                ...
                {
                    //                    TVFineTune.IAMTVAudio a = o as TVFineTune.IAMTVAudio;
                    //                    tuner.TvAudio = a;
                    //#if DEBUG
        ...
        protected void renderGraph()
        {
            ...
            if (wantPreviewRendered && !isPreviewRendered)
            {
                ...
                AMMediaType pmt = new AMMediaType();
                int connectedMediaType = this.sampGrabber.GetConnectedMediaType(pmt);
