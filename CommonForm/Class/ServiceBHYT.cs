...
namespace VNPT.HIS.CommonForm
{
    public class ServiceBHYT
    {
        ...
        //static string URI_HISTORY_010917 = "/api/egw/KQNhanLichSuKCB595";
        //public static wsBHYT_LichSu_respons Get_History(string ma_the, string ho_ten, string ngay_sinh, string gioi_tinh, string ma_CSKCB, string ngay_bd, string ngay_kt)
        //{
        ...
        //    json += Func.json_item("ngaySinh", ngay_sinh.Trim() == "" ? "2017" : ngay_sinh.Trim());                   // ko đc rỗng: phải là dạng dd/MM/yyyy hoặc yyyy
        //    json += Func.json_item("gioiTinh", gioi_tinh == "" ? "1" : gioi_tinh); // biến giới tính ko đc rỗng, phải bằng 1/2
        //    json += Func.json_item("maCSKCB", "49112_BV");// Const.ServiceBHYT_MACSKCB);          // ko đc rỗng
        //    json += Func.json_item("ngayBD", ngay_bd.Trim());
        //    json += Func.json_item("ngayKT", ngay_kt.Trim());
        //    json = "{" + json.Substring(0, json.Length - 1) + "}";

        //    wsBHYT_LichSu_respons KetQua = null;
        //    try
        //    { 
        ...
        //        string url = ServiceBHYT_Url + URI_HISTORY_010917 + strParam;

        //        string ret = RequestPOST(url, json);

        //        KetQua = JsonConvert.DeserializeObject<wsBHYT_LichSu_respons>(ret, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
        ...
        static string URI_HISTORY_010118 = "/api/egw/NhanLichSuKCB2024";
        public static wsBHYT_LichSu_respons_2018 Get_History010118(string ma_the, string ho_ten, string ngay_sinh
            , string gioitinhid="", string noidk = "", string ngaybd = "", string ngaykt = "", string mode = "")
        {
            if (Login() == false) return null;
            ...
            json += Func.json_item("maThe", ma_the.Trim());
            json += Func.json_item("hoTen", ho_ten.Trim());
            json += Func.json_item("ngaySinh", ngay_sinh.Trim() == "" ? "2018" : ngay_sinh.Trim());                   // ko đc rỗng: phải là dạng dd/MM/yyyy hoặc yyyy

            if (gioitinhid.Trim() != "") json += Func.json_item("gioitinhid", gioitinhid.Trim());
            if (noidk.Trim() != "") json += Func.json_item("noidk", noidk.Trim());
            if (ngaybd.Trim() != "") json += Func.json_item("ngaybd", ngaybd.Trim());
            if (ngaykt.Trim() != "") json += Func.json_item("ngaykt", ngaykt.Trim());
            if (mode.Trim() != "") json += Func.json_item("maThe", mode.Trim());
