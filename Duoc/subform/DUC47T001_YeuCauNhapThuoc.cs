...
{
    public partial class DUC47T001_YeuCauNhapThuoc : DevExpress.XtraEditors.XtraForm
    {
        string nhapxuatid;
        string kieu;
        string hinhthuc;
        ...
        public DUC47T001_YeuCauNhapThuoc(string nhapxuatid, string kieu, string hinhthuc, string ht, string edit, string khoid)
        {
            InitializeComponent();
        ...
        private void DUC47T001_YeuCauNhapThuoc_Load(object sender, EventArgs e)
        {
            _initControl();
           // _bindEvent();

            if (this.edit != "1") btnTuChoiPhieu.Visible = true;
        }
        ...
        private void _loadgrid(object sender, EventArgs e)
        {
            ...
            if (page > 0)
            {
                ...
                else if (this.hinhthuc == "12" || this.hinhthuc == "13")
                ...
                if (this.hinhthuc == "13" || this.kieu == "2" || (this.hinhthuc == "12" && this.duyetthuocchan == "0"))
...
//								}
        ...
        private void cmdTaoPhieu_Click(object sender, EventArgs e)
        {
            ...
            if (_check == "1")
            {
                string sql_par = Newtonsoft.Json.JsonConvert.SerializeObject(drugs) + "$" + this.nhapxuatid + "$" + txtGHICHU.Text;
                ...
                if (ret.Rows.Count > 0)
                {
                    if (Func.Parse(ret.Rows[0]["SUCCESS"].ToString()) > 0)
                    {
                        ...
                        {
                            if (ckbBNKhongLayThuoc.Checked == true)
                            {
                                string _par_update = this.nhapxuatid + "$" + (ckbBNKhongLayThuoc.Checked == true ? "1" : "0");
                                var updateBN = RequestHTTP.call_ajaxCALL_SP_I("DUC47T001.11", _par_update);
                            }
                            var _nhapid = ret.Rows[0]["MESSAGE"].ToString().Substring(0, ret.Rows[0]["MESSAGE"].ToString().IndexOf(", "));
                            var _ttxuat = ret.Rows[0]["MESSAGE"].ToString().Substring(ret.Rows[0]["MESSAGE"].ToString().IndexOf(", ") + 1);
                            var _xuatid = _ttxuat.Substring(0, _ttxuat.IndexOf(","));
                            _ttxuat = _ttxuat.Substring(_ttxuat.IndexOf(",") + 1);
                            var _ttnhap = _ttxuat.Substring(0, _ttxuat.IndexOf(","));
                            _ttxuat = _ttxuat.Substring(_ttxuat.IndexOf(",") + 1);
                        ...
                        // 20171020 TuanNM them phan in phieu linh thuoc ngoai tru tu dong
                        var tudongin = RequestHTTP.call_ajaxCALL_SP_S_result("COM.CAUHINH.THEOMA", "PHARMA_TUDONG_IN_PHIEU_LT_NGT");
                        if (tudongin == "1" && this.hinhthuc == "13")
                        {
                            DataTable table = new DataTable();
                            table.Columns.Add("name");
                            table.Columns.Add("type");
                            table.Columns.Add("value");
                            table.Rows.Add("nhapxuatid", "String", this.nhapxuatid);

                            VNPT.HIS.Controls.SubForm.frmPrint frm1 = new Controls.SubForm.frmPrint(table, "DUC004_PHIEUCAPTHUOCNGOAITRU", "pdf", 720, 1200);
                            openForm(frm1);
                        }
                    }
                    else
