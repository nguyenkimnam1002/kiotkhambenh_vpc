...
namespace L1_Mini
{
    public partial class BVVINHPHUC_KIEU3 : Form
    {
        public BVVINHPHUC_KIEU3(bool parent_fullscreen = false)
        {
            InitializeComponent();
        ...
        private void BVVINHPHUC_KIEU3_FormClosed(object sender, FormClosedEventArgs e)
        {
            Return_To_Mainform("dang_nhap_lai", null);
            //frmMain.Current.Exit();
        }

        DataTable dt = new DataTable();
        string he = "1";  // 0: 
        private void BVVINHPHUC_KIEU3_Load(object sender, EventArgs e)
        {
        ...
        private void BVVINHPHUC_KIEU3_SizeChanged(object sender, EventArgs e)
        {
            Load_Button();
        }
        ...
        private void Load_Data()
        {
            ...
            string mode = RequestHTTP.call_ajaxCALL_SP_S_result("COM.CAUHINH", "KIOS_APP_CAPSO_LOAI");
            string param = "";
            if (mode == "0") param = "1$" + Const.local_khoaId;
            else param = "0$" + mode; 

            // mode = 0-truyền ID Khoa thiết lập     phân hệ: 1 - KBH; 2 - VPI; 3 - TIEPNHAN; 4 - CDHA; 5 - THUOC; 6 - XN; 
            ...
            {
                if (mode == "1")
                    MessageBox.Show("Không có dữ liệu cấp số (KBH.CAPSOLAYDL) với khoa "+ Const.local_khoa);
                else
                    MessageBox.Show("Không có dữ liệu cấp số (KBH.CAPSOLAYDL) với tham số " + param + " (1 - KBH; 2 - VPI; 3 - TIEPNHAN; 4 - CDHA; 5 - THUOC; 6 - XN)");
                return;
            }
            ...
            foreach (Control sub_control in Controls)
                if (sub_control.GetType() == typeof(Button))
                {
                    ...
                    btn.Text = (dt.Rows[k]["TEN_HIENTHI"].ToString().Trim() == "" ? "LẤY SỐ" : dt.Rows[k]["TEN_HIENTHI"].ToString()) + "\r\n(" + dt.Rows[k]["STT"].ToString() + ")"; // Name
                    k++;
                }
        }
        private void Load_Button()
        {
            int lbX = (Width - lbTitle.Width) / 2;
            int lbY = lbTitle.Location.Y;
            lbTitle.Location = new System.Drawing.Point(lbX, lbY);
          ...
          //  w = w * 50 / 100;
            ...
            //w = (Width - pading- pading) / number;
            int h = w;
            if (h > Height * 60 / 100) h = Height *60 / 100;
            if (w > h) w = h;
            ...
            for (int k = 0; k < dt.Rows.Count; k++)
            {
                ...
                if (w > 300) btn.Font = new System.Drawing.Font("Tahoma", 60F);
                else btn.Font = new System.Drawing.Font("Tahoma", 28F);
                //btn.UseFont = true;
        ...
        private void BVVINHPHUC_KIEU3_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private bool in_file_stt(string HoTen, string TheBH, string Muc, string STT)
        {
            try
            {
                ...
                string NAME = Report + "_" + DateTime.Now.ToString("yyMMddHHmmss") + "_" + new Random().Next(1000);
        ...
        private string replace_text(string docText, string HoTen, string TheBH, string Muc, string STT)
        {
            ...
            string KIOS_APP_SYT = RequestHTTP.call_ajaxCALL_SP_S_result("COM.CAUHINH", "KIOS_APP_SYT");
            string KIOS_APP_TENBV = RequestHTTP.call_ajaxCALL_SP_S_result("COM.CAUHINH", "KIOS_APP_TENBV");
            docText = docText.Replace("@KIOS_APP_SYT@", KIOS_APP_SYT);
            docText = docText.Replace("@KIOS_APP_BENHVIEN@", KIOS_APP_TENBV);
        ...
        private void BVVINHPHUC_KIEU3_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        { 
            exit = false;
            Return_To_Mainform("opem_kieu1", null); 
        }
    }
}
